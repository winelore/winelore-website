...
type CommissionRole = "EXPERT" | "HEAD_OF_COMMISSION"
...
const MOCK_PARTICIPANTS: Participant[] = [
    { auid: 1, username: "RED has a long name and last", role: "EXPERT", isReady: true, isOnPage: true },
    { auid: 2, username: "RED", role: "EXPERT", isReady: true, isOnPage: true },
    { auid: 3, username: "WHITE has a long name and last name", role: "EXPERT", isReady: false, isOnPage: true },
    { auid: 4, username: "12342314 has a long name and last name", role: "EXPERT", isReady: true, isOnPage: true },
    { auid: 5, username: "1234324", role: "HEAD_OF_COMMISSION", isReady: true, isOnPage: true }, // Виправлено роль
    { auid: 6, username: "324234", role: "EXPERT", isReady: true, isOnPage: false },
    { auid: 7, username: "qwerqwer", role: "EXPERT", isReady: true, isOnPage: true },
    { auid: 8, username: "2134rwdfsa", role: "EXPERT", isReady: true, isOnPage: true },
    { auid: 9, username: "2314asdf", role: "EXPERT", isReady: true, isOnPage: true },
];

const MOCK_COMMISSION: CommissionDetails = {
    id: "comm_123",
    commissionName: "Red Commission",
...
export default function CommissionStartPage() {
    ...
    const [currentUserRole, setCurrentUserRole] = useState<CommissionRole>("EXPERT")
    ...
    useEffect(() => {
        const loadMockData = () => {
            setTimeout(() => {
                ...
                setCommissionDetails(MOCK_COMMISSION)
                setCurrentUserRole("EXPERT") // HEAD_OF_EXPERTS для перевірки кнопки старту
    ...
    const nonReadyCount = participants.filter((p) => !p.isReady).length
                ...
                <div className="mx-auto max-w-7xl flex flex-col lg:flex-row gap-8 lg:gap-12">
                    ...
                    <div className="w-full lg:w-7/12 flex flex-col pt-2">
                        <div className="flex items-start gap-4 mb-6">
                            ...
                            <div>
                                <h2 className="text-3xl font-bold text-card-foreground tracking-tight">{commissionDetails?.commissionName}</h2>
                                <p className="text-lg mt-1">
                                    <span className="font-medium text-emerald-500">{commissionDetails?.status}</span>
                                    <span className="text-muted-foreground/50 mx-2">|</span>
                                    <span className="text-muted-foreground font-medium">{commissionDetails?.timeElapsed}</span>
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-muted-foreground mb-6 bg-card border border-border rounded-xl p-4 w-fit shadow-sm">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="font-medium">{commissionDetails?.competitionName}</span>
                            </div>
                            <span className="text-muted-foreground/50">|</span>
                            <div className="flex items-center gap-1.5">
                                <span className="text-xs">by</span>
                                <User className="h-4 w-4" />
                                <span className="font-medium text-card-foreground">{commissionDetails?.creatorUsername}</span>
                            </div>
                        </div>

                        <p className="text-base text-muted-foreground mb-8 flex items-center gap-2">
                            <Layers className="h-5 w-5" />
                            Featuring <strong className="text-card-foreground">{commissionDetails?.candidateCount}</strong> beverages
                        </p>
